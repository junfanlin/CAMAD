...
class Patient():
    def __init__(self, data_selfr, data_symp, data_dise, block_data, bg, sample_num=50):
        ...
        if args.cuda:
            ...
            self.classifier = Simple_multiclass(66+4, 32, 66)   
            self.classifier.load_state_dict(torch.load('checkpoints/new_exp_models/classifier/'+path, map_location='cpu'))
        ...
        if not all_the_same:
            ...
            with torch.no_grad():
                if args.cuda:
                    ...
                    prop_sample_batch = self.classifier.encode(torch.from_numpy(input_sample_batch))
                    distance = ((prop_sample_batch[0]-prop_sample_batch[1:])**2/prop_sample_batch[1:].var(0)).sum(-1)
                    similarity = torch.nn.functional.softmax(-distance, -1).data.numpy()
        else:
...
class MuzhiEnv():
    ...
    def __init__(self, data_bg, mode='train', tol=11, sample_num=50):
        ...
        self.patient = Patient(data_symp=data_symp, data_selfr=data_selfr, data_dise=data_dise, block_data = data_symp_block, bg=data_bg, sample_num=sample_num)
        self.seed()
        self.reset()
...
class Agent(nn.Module):
    def __init__(self, state_len, action_len, dise_len, emb_len, params):
        ...
        if args.cuda: 
            ...
            self.reclassifier = Simple_multiclass(66, 32, 66)
        ...
        for i in range(self.num_bootstrap):
            if args.cuda:
                ...
                self.classifier3.append(Simple_classifier(66, 8, 4))
            self.optimizer_classifier3.append(torch.optim.Adam(self.classifier3[i].parameters(), lr = 1e-3))
            
        self.classifier3_module = nn.ModuleList(self.classifier3)
    ...
    def sample(self, input_data, mask, reclassifier, sample_num):
        m = torch.distributions.Categorical(logits=reclassifier(input_data))
        samples = []
        for i in range(sample_num):
            samples.append(m.sample()[:, None])
        samples = torch.cat(samples, 1)-1
        samples = samples.float()*(1.-mask[:, None])+input_data[:, None]*mask[:, None]
        return samples
    
    def cal_prop_uncertainty(self, input_data, mask, sample_num = 1):
        samples = self.sample(input_data, mask, self.reclassifier, sample_num=sample_num)
        props = []
        for i in range(self.num_bootstrap):
            props.append(torch.nn.functional.softmax(self.classifier3[i](samples.view(-1, 66)), -1).view(-1, sample_num, 4))
    ...
    def new_state(self, s_t, verbose=False):
        with torch.no_grad():
            prop_mean, prop_std = self.cal_prop_uncertainty(s_t[:, :66], s_t[:, 66:132], sample_num=self.sample_num)
    ...
    def compute_loss(self, s_t, act, rew, done, s_tplus1):
        ...
        with torch.no_grad():
            ...
            q_vals_targ = rew + (1-done)*self.gamma*(q_vals_next.detach().max(1)[0].view(-1, 1)) 
...
#             print("Updating ... ")
    ...
    def update_checker(self, verbose=False):
        ...
        for i in range(self.num_bootstrap):
            ...
            with torch.no_grad():
                samples = self.sample(input_data, input_mask, self.reclassifier, sample_num=sample_num).detach()

            self.optimizer_classifier3[i].zero_grad()
            props_logits = self.classifier3[i](samples.view(-1, 66)).view(-1, sample_num, 4).mean(1)
            ce_loss = self.criterion_class(props_logits, input_label)
            ce_loss = self.criterion_class(self.classifier3[i](input_data), input_label) + ce_loss
            ce_loss.backward()
    ...
    for i_episode in range(len(patience.goals)):
        ...
        while True:
            if args.cuda:
                ...
                obs_tensor = torch.from_numpy(obs.reshape(1, -1))
