from nips2019 import *
import argparse


parser = argparse.ArgumentParser()

parser.add_argument('--reward', help='gs host', default=0.1, type=float)
parser.add_argument('--gamma', default=0.95, type=float)
parser.add_argument('--seed', default=0, type=int)
parser.add_argument('--sample_num', default=50, type=int)
parser.add_argument('--num_bootstrap', default=10, type=int)
...
params['num_bootstrap'] = args.num_bootstrap
params['dueling'] = True
params['seed'] = args.seed
params['sample_num'] = args.sample_num
params['block'] = False

params['logger_name'] = 'my_train_test_data_bgall_noblock3-22_push_'+str(params['seed'])+'_'+str(params['reward'])+'_'+str(params['gamma'])+'_'+str(params['num_bootstrap'])+'_'+str(params['sample_num'])#'my_all_block_log'
import logging
...
patience = MuzhiEnv(data_bg='train', tol=params['max_turn'], sample_num=params['sample_num'])
patience_test = MuzhiEnv(mode='test', data_bg='test_data_bgall', tol=params['max_turn'], sample_num=params['sample_num'])
...
for i_episode in range(params['num_episode']):
        ...
        if done:
            if True:
                ...
                if len(doctor.memory_checker) > params['batch_size']:
                    doctor.update_reclassifier(verbose=verbose)
    ...
    if ((i_episode) % params['test_freq'] == 0):
        ...
        print((doctor.classifier3[0](torch.from_numpy(patience_test.patient.sympbase)).max(1)[1] == torch.from_numpy(patience_test.patient.disebase)).float().mean())
